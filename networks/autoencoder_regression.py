# Autoencoder regression algorithm
import numpy as np
...
class AutoencoderRegressor:
    ...
    def fit(self, X, y):
        ...
        print('Autoencoding layer trained')
        ...
        self.w_c[1] = np.random.uniform(-0.01, 0.01, (self.cls_layer + 1))
                ...
                for j in reversed(range(1, len(w))):
                    ...
                    for k in range(len(err)):
                        for m in range(len(w[j])):
                            next_error = next_error + err[k] * w[j][m]

                    # Calculate weight change
                    if j == len(w) - 1:
                        for k in range(len(err)):
                            delta = self.learning_rate * np.sum(err[k] * z[j - 1])
                            w[j][k] = w[j][k] + delta
                    else:
                        for k in range(len(w[j])):
                            delta = self.learning_rate * z[j - 1] * np.sum(err[k] * z[j] * (1 - z[j]))
                            w[j][k] = w[j][k] + delta
    ...
    def fit_autoencoder(self, X):
        ...
        while (not convergence_reached) and epochs < self.max_iter:
            ...
            for i in range(len(indices)):
                ...
                error = error + mean_squared_error(prediction, _X[i])
    ...
    def calculate_output(x, w):
        ...
        z[len(z) - 1] = np.dot(w[len(w) - 1], z[len(z) - 2])
    ...
    def predict(self, X):
        ...
        for i in range(len(_X)):
            predicted.append(self.calculate_output(_X[i], self.w)[-1])
