...
class Sarsa:
    ...
    def fit(self, starting_positions, num_episodes=1000, verbose=False):
        ...
        while episodes <= num_episodes:
            ...
            while not won:
                ...
                self.q[(state, action)] = old_q + self.eta * (reward + self.gamma * self.q[(new_state, next_action)]
                                                              - old_q)
    ...
    def generate_learning_curve(self, starting_position, track_name, num_episodes=10000):
        ...
        plt.title(f"Sarsa {track_name}")
        plt.xlabel('Episode #')
        plt.ylabel('Iterations')

        # Save graph to image file
        mod_track_name = track_name.lower()
        plt.savefig(f"sarsa_{mod_track_name}_learning_curve.png", bbox_inches='tight')
